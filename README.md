...
**Nothing untested is installed.** Every build is published as a pre-release, and a build installs only once a hardware test on the TrueNAS train it was built for signs it off (closing its hardware-test issue as completed writes a `verified-train` marker into its release notes). A 26 sign-off approves a build for TrueNAS 26 boxes only; a 25.10 box keeps the newest build verified on 25.10. Releases promoted before per-train sign-off count for every train. The train is the major version from 26 on (every 26.x release, betas included, is train 26) and major.minor before that (25.10). When nothing is approved for your TrueNAS version yet, `get.sh` says so and names the hardware test that is waiting for a tester.

> **Older one-liners:** `.../releases/latest/download/install.sh` still works, but it runs whatever installer the current **Latest** release carries, which predates per-train approval. Use `get.sh` below.
...
| Preview (beta) | 26.0.0-BETA.3 (Halfmoon) | `6.18.42-production+truenas` | MemryX SDK 2.1 | [`v26.0.0-BETA.3-memryx2.1-r15`](https://github.com/truenas-community-sysexts/memryx-mx3-support/releases/tag/v26.0.0-BETA.3-memryx2.1-r15) |
...
Detects your TrueNAS version and train, picks the newest release approved for
them, checks its image against the release checksum, runs that release's
installer, sets up persistence, loads the module, and starts the
`mxa-manager` daemon:
...
With an explicit pool for persistence (any installer option goes after
`bash -s --`):
...
> **Version matching:** each release is built for a specific TrueNAS version's
> kernel. `get.sh` auto-detects your version and TrueNAS train and downloads
> the correct approved release. `--release=TAG` installs that exact release
> instead (see [docs/install.md](docs/install.md)).
...
```bash
curl -fsSL https://raw.githubusercontent.com/truenas-community-sysexts/memryx-mx3-support/main/get.sh | sudo bash -s -- --check
```
